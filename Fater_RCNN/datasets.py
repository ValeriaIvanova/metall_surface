...
class CustomDataset(Dataset):
    def __init__(
        ...
        # получить все пути к изображениям в отсортированном порядке
        for file_type in self.image_file_types:
            self.all_image_paths.extend(glob.glob(os.path.join(self.images_path, file_type)))
        self.all_annot_paths = glob.glob(os.path.join(self.labels_path, '*.xml'))
        self.all_images = [image_path.split(os.path.sep)[-1] for image_path in self.all_image_paths]
        self.all_images = sorted(self.all_images)
        # Удалите все аннотации и изображения, если объект отсутствует.
        self.read_and_clean()
    ...
    def read_and_clean(self):
        ...
        # Удалите любой файл изображения, если нет файла аннотаций
        # не найден для изображения.
    ...
    def load_image_and_labels(self, index):
        ...
        # Считывание соответствующего XML-файла для получения аннотаций.
        ...
        # Координаты полей для xml-файлов извлекаются и корректируются в соответствии с заданным размером изображения.
        for member in root.findall('object'):
            # Сопоставьте текущее имя объекта со списком "classes", чтобы получить
            # индекс метки и добавьте в список `lable`.
            ...
            # Изменние размера ограничивающих рамок в соответствии с
            # желаемой `шириной`, `высотой`.
        ...
        # Ограничивающая рамка для тензора.
        boxes = torch.as_tensor(boxes, dtype=torch.float32)
        # Область ограничивающих прямоугольников.
    ...
    def check_image_and_annotation(self, xmax, ymax, width, height):
        """
        Проверка, что все x_max и y_max не превышают размер изображения
        ширина или высота.
        """
        if ymax > height:
            ymax = height
