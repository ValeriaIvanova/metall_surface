...
def collect_all_images(dir_test):
    ...
    параметр dir_test: Каталог, содержащий изображения, или путь к одному изображению.

    Returns:
        test_images: список, содержащий все пути к изображениям.
    """
    test_images = []
...
def parse_opt():
    # Функция для запуска обученной модели
    parser = argparse.ArgumentParser()
    parser.add_argument(
        '-i', '--input',
        help='folder path to input input image',
    )
    parser.add_argument(
        '-c', '--config',
        default=None,
...
def main(args):
    ...
    # Определение порога точности обнаружения для любого класса,
    # если порог ниже, то пропуск определения.
    detection_threshold = args['threshold']

    # Для подсчета общего количества пройденных кадров.
    frame_count = 0
    # Добавление частоты кадров в секунду
    total_fps = 0
    for i in range(len(test_images)):
        # Получение имя файла изображения для последующего сохранения выходных данных.
